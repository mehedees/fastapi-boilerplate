...
def test_generate_and_decode_token_roundtrip():
    tu = TokenUtils(SECRET, ALG)
    token, iat = tu.generate_token(
        {"user_id": 1, "email": "a@b.com"}, TokenTypeEnum.ACCESS, 3600
    )
    assert isinstance(token, str)
    assert iat == datetime(2025, 1, 1, 0, 0, 0, tzinfo=UTC)

    payload = tu.decode_token(token)
...
def test_decode_expired_token_raises():
    tu = TokenUtils(SECRET, ALG)
    token, _ = tu.generate_token({}, TokenTypeEnum.ACCESS, 1)
    with freeze_time("2025-01-01 01:00:00"):
        import pytest

        with pytest.raises(jwt.ExpiredSignatureError):
            tu.decode_token(token)
